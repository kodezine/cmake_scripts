...
# the configuration of hal should be available in all cases if it is not provided, we use ALL the available drivers in
# the default configuration
if (EXISTS STM32CubeMxConfigHeaderFile)
  message (STATUS "${libName}: using defined header file, ${STM32CubeMxConfigHeaderFile}")
  configure_file (
    ${STM32CubeMxConfigHeaderFile}
    ${${libName}_SOURCE_DIR}/Drivers/STM32${UPPERCASE_STM32_TYPE}xx_HAL_Driver/Inc/stm32${LOWERCASE_STM32_TYPE}xx_hal_conf.h
    COPYONLY)
else ()
  message (STATUS "${libName}: using all available HAL layer artefacts")
  configure_file (
    ${${libName}_SOURCE_DIR}/Drivers/STM32${UPPERCASE_STM32_TYPE}xx_HAL_Driver/Inc/stm32${LOWERCASE_STM32_TYPE}xx_hal_conf_template.h
    ${${libName}_SOURCE_DIR}/Drivers/STM32${UPPERCASE_STM32_TYPE}xx_HAL_Driver/Inc/stm32${LOWERCASE_STM32_TYPE}xx_hal_conf.h
    COPYONLY)
endif ()
...
set (
  st_cmsis_PATH
  ${${libName}_SOURCE_DIR}/Drivers/CMSIS
  CACHE PATH "Path to STM32CubeXX CMSIS folder" FORCE)
...
set (
  st_HAL_Driver_DIR
  ${${libName}_SOURCE_DIR}/Drivers/STM32${UPPERCASE_STM32_TYPE}xx_HAL_Driver
  CACHE PATH "Path to STM32CubeXX Drivers folder" FORCE)
set (
  stm32_hal
  stm32${LOWERCASE_STM32_TYPE}xx_hal
  CACHE STRING "Prefix for HAL files")
...
target_sources (${libName} PRIVATE ${st_HAL_DRV_SOURCES})

target_include_directories (
  ${libName}
  PUBLIC $<BUILD_INTERFACE:${cmsis_INCLUDE_PATH}>
