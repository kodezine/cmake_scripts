...
function (setTargetCompileOptions PROJECTNAME)
  target_compile_options (
    ${${PROJECTNAME}}
    PUBLIC ${COMPILER_SPECIFIC_CFLAGS}
           # -std=c11
           -c
           -fno-rtti
           -funsigned-char
...
function (setTargetLinkOptions PROJECTNAME)
  ...
  add_custom_command (
    TARGET ${${PROJECTNAME}}
    POST_BUILD
    COMMAND ${CMAKE_OBJCOPY} -O ihex $<TARGET_FILE:${${PROJECTNAME}}> ${${PROJECTNAME}}.hex)
  add_custom_command (
    TARGET ${${PROJECTNAME}}
    POST_BUILD
    COMMAND ${CMAKE_OBJCOPY} -O binary $<TARGET_FILE:${${PROJECTNAME}}> ${${PROJECTNAME}}.bin)

  add_custom_command (
    TARGET ${${PROJECTNAME}}
    POST_BUILD
    COMMAND ${CMAKE_COMMAND} -E echo "Object file: $<TARGET_FILE_DIR:${${PROJECTNAME}}>/${${PROJECTNAME}}.obj.txt"
    COMMAND ${CMAKE_LLVM_OBJDUMP} --dwarf=frames $<TARGET_FILE:${${PROJECTNAME}}> > ${${PROJECTNAME}}.obj.txt)
  add_custom_command (
    TARGET ${${PROJECTNAME}}
    POST_BUILD
    COMMAND ${CMAKE_COMMAND} -E echo "Symbol file: $<TARGET_FILE_DIR:${${PROJECTNAME}}>/${${PROJECTNAME}}.nm.txt"
    COMMAND ${CMAKE_LLVM_NM} --print-size --size-sort --line-numbers $<TARGET_FILE:${${PROJECTNAME}}> >
            ${${PROJECTNAME}}.nm.txt)
  add_custom_command (
    TARGET ${${PROJECTNAME}}
    POST_BUILD
    COMMAND ${CMAKE_COMMAND} -E echo "Dwarf file : $<TARGET_FILE_DIR:${${PROJECTNAME}}>/${${PROJECTNAME}}.dwarf.txt"
    COMMAND ${CMAKE_LLVM_READOBJ} -a $<TARGET_FILE:${${PROJECTNAME}}> > ${${PROJECTNAME}}.dwarf.txt)
