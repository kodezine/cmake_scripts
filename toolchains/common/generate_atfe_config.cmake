...
#
# This module provides a CMake function to dynamically generate ATFE (Arm Toolchain for Embedded) configuration files
# based on runtime library, target architecture, and CRT variant selections.
#
# The generator function: 1. Validates COMPILER_ROOT_PATH and clang binary existence (fail early) 2. Detects sysroot
# location using dual-pattern support 3. Auto-downloads missing runtime overlays from ARM releases (if enabled) 4.
# Composes config file with appropriate flags for target/runtime/CRT 5. Only regenerates when content changes (MD5-based
# check)
#
# Generated config files are placed in toolchains/common/.atfe_config/ which is git-ignored to prevent accidental
# commits.
...
#
# Parameters: COMPILER_ROOT_PATH - Path to ATFE installation (required, must contain bin/clang) TARGET_ARCH        -
# Target architecture triple (e.g., armv6m-none-eabi, armv7em-none-eabi) MARCH              - Architecture flag (e.g.,
# armv6-m, armv7e-m) MCPU               - CPU flag (e.g., cortex-m0, cortex-m4, cortex-m7) MFLOAT_ABI         - Float
# ABI (soft, softfp, hard) MFPU               - FPU type (none, fpv4-sp-d16, fpv5-d16) RUNTIME_LIB        - Runtime
# library (newlib, newlib-nano, picolibc) CRT_VARIANT        - CRT variant (default, semihost, nosys) AUTO_DOWNLOAD -
# Enable auto-download of missing overlays (ON/OFF) OUTPUT_VAR         - Variable name to store generated config file
# path
#
function (generate_atfe_config)
  set (options "")
  ...
  # ============================================================================ EARLY VALIDATION - Fail fast with clear
  # error messages
  # ============================================================================
  ...
  if (NOT EXISTS "${ATFE_COMPILER_ROOT_PATH}")
    message (FATAL_ERROR "ATFE Configuration Error: COMPILER_ROOT_PATH does not exist.\n"
  ...
  elseif (EXISTS "${SYSROOT_PATTERN2}")
    ...
    if (ATFE_AUTO_DOWNLOAD)
      ...
      if (CLANG_VERSION_OUTPUT MATCHES "version ([0-9]+)\\.([0-9]+)")
        ...
        message (FATAL_ERROR "ATFE Configuration Error: Failed to detect clang version.\n"
      ...
      elseif (CMAKE_HOST_SYSTEM_NAME STREQUAL "Darwin")
        ...
        message (FATAL_ERROR "ATFE Configuration Error: Unsupported host platform: ${CMAKE_HOST_SYSTEM_NAME}\n"
      ...
      set (OVERLAY_URL
           "https://github.com/arm/arm-toolchain/releases/download/release-${CLANG_VERSION}.1-ATfE/${OVERLAY_FILENAME}")
      set (CHECKSUM_URL
           "https://github.com/arm/arm-toolchain/releases/download/release-${CLANG_VERSION}.1-ATfE/SHA256SUMS")
      ...
      file (
        DOWNLOAD "${OVERLAY_URL}" "${ARCHIVE_PATH}"
      ...
      if (NOT DOWNLOAD_STATUS_CODE EQUAL 0)
        ...
        message (
          FATAL_ERROR "ATFE Configuration Error: Failed to download overlay.\n" "URL: ${OVERLAY_URL}\n"
      ...
      if (NOT EXTRACT_RESULT EQUAL 0)
        message (FATAL_ERROR "ATFE Configuration Error: Failed to extract overlay.\n" "Archive: ${ARCHIVE_PATH}\n"
  ...
  file (RELATIVE_PATH SYSROOT_RELATIVE "${ATFE_COMPILER_ROOT_PATH}/bin" "${SYSROOT_PATH}")
