...
# https://interrupt.memfault.com/blog/arm-cortexm-with-llvm-clang#update-clang-baremetal check the environment variable
# for the supported cortex type
...
if (NOT EXISTS "$ENV{ARM_GCC_ROOT_FOLDER}")
  ...
  message (
    FATAL_ERROR
      "The cross compile toolchain with clang did not find a suitable arm-none-eabi-gcc to work with, aborting!")
else ()
  message (CHECK_PASS "found ... \"$ENV{ARM_GCC_ROOT_FOLDER}\"")
  set (ARM_GCC_ROOT_FOLDER "$ENV{ARM_GCC_ROOT_FOLDER}")
  execute_process (
    COMMAND ${ARM_GCC_ROOT_FOLDER}/bin/arm-none-eabi-gcc${TC_POSTFIX} ${cpu_mode} ${cpu_flag} ${float_abi} ${fpu_type}
            -print-sysroot
...
# set target compiler triplet (throws error otherwise) We do not use the default config files since we use the libraries
# from GCC set(FLAGS "${llvm_config_file}" CACHE STRING "Compiler flags")
