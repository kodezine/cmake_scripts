...
function (generate_object target newsuffix type)
  add_custom_command (
    ...
    COMMAND ${CMAKE_OBJCOPY} -O${type} "${CMAKE_CURRENT_BINARY_DIR}/${target}${CMAKE_EXECUTABLE_SUFFIX}"
            "${CMAKE_CURRENT_BINARY_DIR}/${target}${newsuffix}")
endfunction ()
...
function (generate_lst target)
  add_custom_command (
    TARGET ${target}${SUFFIX}
    POST_BUILD
    COMMAND ${CMAKE_OBJDUMP} -St "${CMAKE_CURRENT_BINARY_DIR}/${target}${CMAKE_EXECUTABLE_SUFFIX}"
...
function (generate_mocks mock_headers working_dir perl_sh_dir create_mocks_dir)
  ...
  foreach (element IN LISTS DEBOUNCER_AO_MOCKS)
    execute_process (COMMAND cp ${element} ${working_dir})
  endforeach ()
  file (GLOB MOCKRAW ${working_dir}/*.h)
  foreach (element IN LISTS MOCKRAW)
    execute_process (COMMAND sh ${perl_sh_dir}/perl.sh ${element} ${element}.tmp)
    execute_process (COMMAND mv ${element}.tmp ${element})
  endforeach ()

  foreach (element IN LISTS MOCKRAW)
    execute_process (COMMAND ruby ${create_mocks_dir}/create_mocks.rb ${element})
  endforeach ()
